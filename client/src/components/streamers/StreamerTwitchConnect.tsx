...
interface StreamerTwitchConnectProps {
  streamerId: number;
  hasLinkedAccount: boolean;
}

export function StreamerTwitchConnect({ streamerId, hasLinkedAccount }: StreamerTwitchConnectProps) {
  ...
  const linkTwitchAccount = async () => {
    setIsLinking(true);
    try {
      const res = await fetch(`/api/twitch/auth-url?streamerId=${streamerId}`);
  ...
  const unlinkMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest('POST', `/api/twitch/unlink-account`, { streamerId });
      if (!response.ok) {
      ...
      <CardContent>
        {hasLinkedAccount ? (
          <div className="bg-green-500/10 text-green-500 p-3 rounded flex items-center gap-2">
            ...
            <p className="text-sm">Your Twitch account is connected</p>
          </div>
        ) : (
